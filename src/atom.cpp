...
void Atom::set_atomflag_defaults()
{
  ...
  q_flag = mu_flag = 0;
  //new variables
  qreward_flag = poidsnn_flag = dreward_flag = dpoids_flag = lightintensity_flag = clock_flag = 0;
...
void *Atom::extract(const char *name)
{
  ...
  if (strcmp(name,"clock") == 0) return (void *) clock; 
...
int Atom::extract_datatype(const char *name)
{
  ...
  if (strcmp(name,"clock") == 0) return LAMMPS_DOUBLE;
